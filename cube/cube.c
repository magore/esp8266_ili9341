 ...
 *      Author: Sem
 *  Mike Gore 2015 - Fixed Cordic code to handle any angle size
 */
#include "user_config.h"
#include <math.h>
#include "cube.h"
											// X, Y, Z pairs - 8 x,y,z points on a CUBE - so 8 x 3
...
void cube_calculate(double degreeX, double degreeY, double degreeZ, double scale, int16_t shiftX, int16_t shiftY, int16_t shiftZ)
{
	...
	cordic_deg(degreeX, &sinx, &cosx);
	cordic_deg(degreeY, &siny, &cosy);
	cordic_deg(degreeZ, &sinz, &cosz);
